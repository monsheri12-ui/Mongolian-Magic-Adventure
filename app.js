...
const audioManifest=await fetch('audio.json?v=20260925-1').then(r=>{if(!r.ok)throw Error('audio');return r.json();}).catch(()=>null);
const quizData=await fetch('quizzes.json?v=20260925-1').then(r=>{if(!r.ok)throw Error('quiz');return r.json();});
