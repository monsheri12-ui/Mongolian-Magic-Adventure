...
(async()=>{
 ...
 const data=await fetch('chapter.json?v=20260925-2').then(r=>{if(!r.ok)throw Error('Не удалось загрузить главу');return r.json();}),art={"bakery-alarmed": "art-bakery-alarmed.webp", "bakery-approach": "art-bakery-approach.webp", "bakery-name": "art-bakery-name.webp", "breakfast": "art-breakfast.webp", "cat-corner": "art-cat-corner.webp", "cat-watch": "art-cat-watch.webp", "cloaked-street": "art-cloaked-street.webp", "daylight-owls": "art-daylight-owls.webp", "departure": "art-departure.webp", "family": "art-family.webp", "house": "art-house.webp", "office": "art-office.webp", "office-hesitation": "art-office-hesitation.webp", "office-window": "art-office-window.webp", "owl": "art-owl.webp", "petunia": "art-petunia.webp", "potters": "art-potters.webp", "secret": "art-secret.webp", "purple-greeting": "art-purple-greeting.webp", "evening-news": "art-evening-news.webp", "night-cat": "art-night-cat.webp", "night-dumbledore": "art-night-dumbledore.webp", "night-professors": "art-night-professors.webp", "hagrid-arrival": "art-hagrid-arrival.webp", "baby-doorstep": "art-baby-doorstep.webp", "ch2-cupboard-night": "art-ch2-cupboard-night.webp", "ch2-reptiles": "art-ch2-reptiles.webp", "ch2-birthday": "art-ch2-birthday.webp", "ch2-cupboard": "art-ch2-cupboard.webp", "ch2-zoo": "art-ch2-zoo.webp", "ch2-escape": "art-ch2-escape.webp", "ch2-car": "art-ch2-car.webp", "ch2-roof": "art-ch2-roof.webp", "ch2-morning-house": "art-ch2-morning-house.webp", "ch2-family-photos": "art-ch2-family-photos.webp"};
 ...
 function savedDetail(w){
  ...
  if(w.kind==='legacy')return {lemma:w.lemma,ru:w.ru.replace(/Харри(?![а-яёөү])/g,'Гарри'),note:'Слово из предыдущей версии игры.',context:''};
 ...
 function renderPlay(){const s=getScene();$('#cn-shot').dataset.asset=s.asset;renderOtherForms();
  ...
  const chapter=chapterOf(s),range=chapterRange(chapter),progress=chapterProgress(chapter),atEnd=state.scene===range.last,complete=progress.percent===100,nextChapter=data.scenes[range.last+1];
  $('#cn-chapter-select').innerHTML=availableChapters().map(c=>`<option value="${c}" ${c===chapter?'selected':''}>${c}. ${esc(data.chapters[c-1].title)}</option>`).join('');
  $('#cn-back').disabled=state.scene===0;
  $('#cn-next').textContent=atEnd?(complete?(nextChapter?`Перейти к главе ${chapterOf(nextChapter)} →`:'Перечитать главу ↻'):'Завершить главу →'):'Продолжить историю →';
  $('#cn-progress').textContent=`Глава ${chapter} · прочитано ${progress.percent}%`;
  $('#cn-reading-position').textContent=`Глава ${chapter} · ${sentenceLabel(s)} · отрывок ${state.scene-range.first+1} из ${range.count}`;
  $('#cn-end').hidden=!atEnd;
  $('#cn-end').innerHTML=`<h3>Конец ${chapter===1?'первой':chapter===2?'второй':chapter+'-й'} главы</h3><p>Гарри Поттер и философский камень · Глава ${chapter}. ${esc(data.chapters[chapter-1].title)}</p><p>${esc(data.chapters[chapter-1].endSummary||'')}</p><p>${complete?`Глава прочитана · 100%. Вы прошли все ${range.count} отрывков.`:state.read.includes(s.id)?`Последний отрывок отмечен. Прочитано ${progress.percent}% главы; пропущенные места доступны в «Хронике».`:'Это последняя строка главы. Нажмите «Завершить главу», чтобы отметить этот отрывок прочитанным.'}</p><button type="button" class="cn-primary" data-view="words">Повторить мои слова ${icon('arrow-right')}</button>${nextChapter?`<button type="button" class="cn-secondary" data-scene="${range.last+1}">Глава ${chapterOf(nextChapter)}. ${esc(data.chapters[chapterOf(nextChapter)-1].title)} →</button>`:''}`;
 ...
 const availableChapters=()=>[...new Set(data.scenes.map(chapterOf))];
 function chapterRange(chapter){const indices=data.scenes.map((s,i)=>chapterOf(s)===chapter?i:-1).filter(i=>i>=0);return {first:indices[0],last:indices[indices.length-1],count:indices.length};}
 ...
 function navigate(index){state.chapterPositions={...(state.chapterPositions||{}),[chapterOf(getScene())]:state.scene,[chapterOf(data.scenes[index])]:index};state.scene=index;state.storyChapter=chapterOf(getScene());state.furthest=Math.max(state.furthest,index);state.view='play';help=null;selected=null;status='';save();render();}
 root.addEventListener('change',e=>{if(e.target.id==='cn-chapter-select'){const chapter=Number(e.target.value),range=chapterRange(chapter),remembered=state.chapterPositions?.[chapter];navigate(Number.isInteger(remembered)&&remembered>=range.first&&remembered<=range.last?remembered:range.first);return;}if(e.target.id==='cn-level'){state.level=Number(e.target.value);selected=null;status='';save();render();}});
 root.addEventListener('click',e=>{const b=e.target.closest('button');if(!b||b.disabled||!root.contains(b))return;
  ...
  if(b.dataset.view){if(b.dataset.view==='story')state.storyChapter=chapterOf(getScene());state.view=b.dataset.view;status='';save();render();return;}
  ...
  if(b.id==='cn-next'){const range=chapterRange(chapterOf(getScene())),atEnd=state.scene===range.last,alreadyRead=state.read.includes(getScene().id);if(!alreadyRead)state.read.push(getScene().id);state.finished=state.read.length===TOTAL;if(!atEnd){navigate(state.scene+1);}else if(!alreadyRead){help=null;selected=null;save();render();}else if(range.last<LAST){navigate(range.last+1);}else{navigate(range.first);}return;}
...
const audioManifest=await fetch('audio.json?v=20260925-2').then(r=>{if(!r.ok)throw Error('audio');return r.json();}).catch(()=>null);
const quizData=await fetch('quizzes.json?v=20260925-2').then(r=>{if(!r.ok)throw Error('quiz');return r.json();});
...
async function loadAudioPack(name){if(!audioPacks.has(name)){const revision=audioManifest?.bundles?.find(pack=>pack.file===name)?.sha256||'20260925-2';const promise=fetch(name+'?v='+encodeURIComponent(revision)).then(r=>{if(!r.ok)throw Error('audio download');return r.arrayBuffer();}).catch(e=>{audioPacks.delete(name);throw e;});audioPacks.set(name,promise);if(audioPacks.size>3)audioPacks.delete(audioPacks.keys().next().value);}return audioPacks.get(name);}
let quizChapter=1,quizLevel=state.level,quizIndex=0,quizSummary=false,quizDraft=null;
...
function renderAudio(){
 ...
 const host=$('#cn-audio');
 if(!c){host.innerHTML=audioManifest?'<p>Озвучка этой главы ещё не добавлена.</p>':'<p>Озвучка пока недоступна. Попробуйте обновить страницу.</p>';return;}
...
function quizQuestions(level=quizLevel,chapter=quizChapter){return quizData.chapters[chapter].levels[level];}
function quizStore(level=quizLevel,chapter=quizChapter){state.chapterQuiz??={version:2,chapters:{}};state.chapterQuiz.chapters??={};const ch=state.chapterQuiz.chapters[chapter]??={levels:{}};return ch.levels[level]??={answers:[],best:0};}
function validatedQuiz(value){
 const out={version:2,chapters:{}};if(!value||![1,2].includes(value.version))return out;
 const chapters=value.version===1?{1:{levels:value.levels}}:value.chapters;
 for(const c of availableChapters()){const old=chapters?.[c];if(!old)continue;const ch=out.chapters[c]={levels:{}};for(let l=0;l<3;l++){const src=old.levels?.[l];if(!src)continue;const qs=quizQuestions(l,c);ch.levels[l]={answers:qs.map((q,i)=>{const a=src.answers?.[i];return a&&Number.isInteger(a.choice)&&a.choice>=0&&a.choice<q.options.length&&a.checked===true?{choice:a.choice,checked:true}:null;}),best:Number.isInteger(src.best)?Math.max(0,Math.min(qs.length,src.best)):0};}}return out;
}
function openQuiz(level=state.level,chapter=chapterOf(getScene())){quizChapter=chapter;quizLevel=level;state.quizChapter=chapter;state.quizLevel=level;quizDraft=null;quizSummary=false;quizIndex=Math.max(0,quizQuestions(level).findIndex((_,i)=>!quizStore(level).answers[i]?.checked));state.view='quiz';save();render();$('#cn-quiz-heading')?.focus({preventScroll:true});}
function renderQuiz(){
 const qs=quizQuestions(),store=quizStore(),checked=store.answers.filter(a=>a?.checked).length;
 const score=qs.reduce((n,q,i)=>n+(store.answers[i]?.checked&&store.answers[i].choice===q.correct?1:0),0);
 const header=`<h2 id="cn-quiz-heading" tabindex="-1">Понимание главы ${quizChapter}</h2><p>Гарри Поттер и философский камень · ${esc(data.chapters[quizChapter-1].title)}</p><label class="cn-quiz-chapter">Глава <select id="cn-quiz-chapter" aria-label="Глава итогового теста">${availableChapters().map(c=>`<option value="${c}" ${c===quizChapter?'selected':''}>${c}. ${esc(data.chapters[c-1].title)}</option>`).join('')}</select></label><div class="cn-quiz-levels" role="group" aria-label="Уровень итогового теста">${levels.map((l,i)=>`<button type="button" class="cn-secondary" data-quiz-level="${i}" aria-pressed="${i===quizLevel}">${l}</button>`).join('')}</div>`;
 if(quizSummary&&checked===qs.length){
  store.best=Math.max(store.best,score);
  $('#cn-quiz').innerHTML=header+`<div class="cn-quiz-result"><span>Глава ${quizChapter} · ${levels[quizLevel]}</span><h3>${score} из ${qs.length} · ${Math.round(score/qs.length*100)}%</h3><p>${score===qs.length?'Все ответы верны. Вы разобрались в событиях и деталях главы.':'Вернитесь к отрывкам, в которых возникли трудности, и попробуйте ещё раз.'}</p><p>Лучший результат: ${store.best} из ${qs.length}. Прогресс чтения считается отдельно.</p></div><ol class="cn-quiz-review">${qs.map((q,i)=>`<li><button type="button" data-quiz-review="${i}">${store.answers[i].choice===q.correct?'✓ Верно':'↻ Повторить'} · ${esc(q.prompt)}</button></li>`).join('')}</ol><div class="cn-quiz-actions"><button type="button" class="cn-primary" id="cn-quiz-retry">Пройти заново</button><button type="button" class="cn-secondary" data-view="words">Повторить мои слова</button><button type="button" class="cn-secondary" data-view="play">Вернуться к чтению</button></div>`;return;
 }
...
function validateProgress(raw){
 ...
 if(s.chapterQuiz!==undefined){const v=s.chapterQuiz;if(!v||![1,2].includes(v.version))throw Error('Повреждены результаты теста.');const chapters=v.version===1?{1:{levels:v.levels}}:v.chapters;if(!chapters||typeof chapters!=='object'||Array.isArray(chapters))throw Error('Повреждены результаты теста.');for(const [c,ch]of Object.entries(chapters)){if(!quizData.chapters[c]||!ch?.levels||typeof ch.levels!=='object'||Array.isArray(ch.levels))throw Error('Повреждены результаты теста.');for(const [l,rec]of Object.entries(ch.levels)){const qs=quizData.chapters[c].levels[l];if(!['0','1','2'].includes(l)||!rec||!Array.isArray(rec.answers)||rec.answers.length>qs.length||rec.best!==undefined&&(!Number.isInteger(rec.best)||rec.best<0||rec.best>qs.length))throw Error('Повреждены результаты теста.');for(const [i,a]of rec.answers.entries())if(a!==null&&(!a||a.checked!==true||!Number.isInteger(a.choice)||a.choice<0||a.choice>=qs[i].options.length))throw Error('Повреждён ответ в тесте.');}}}
...
root.addEventListener('click',e=>{const b=e.target.closest('button');if(!b||b.disabled)return;
 ...
 else if(b.dataset.quizLevel!==undefined)openQuiz(+b.dataset.quizLevel,quizChapter);
 else if(b.dataset.quizChoice!==undefined){quizDraft=+b.dataset.quizChoice;renderQuiz();}
 else if(b.id==='cn-quiz-check'&&quizDraft!==null){quizStore().answers[quizIndex]={choice:quizDraft,checked:true};quizDraft=null;const qs=quizQuestions();if(qs.every((_,i)=>quizStore().answers[i]?.checked))quizStore().best=Math.max(quizStore().best,qs.reduce((n,q,i)=>n+(quizStore().answers[i].choice===q.correct?1:0),0));save();renderQuiz();}
 else if(b.id==='cn-quiz-next'){if(quizIndex<quizQuestions().length-1)quizIndex++;else quizSummary=true;quizDraft=null;renderQuiz();}
...
root.addEventListener('change',async e=>{if(e.target.id==='cn-quiz-chapter'){openQuiz(quizLevel,+e.target.value);}else if(e.target.id==='cn-audio-speed'){audioSpeed=+e.target.value;narration.playbackRate=audioSpeed;}else if(e.target.id==='cn-import-progress'&&e.target.files[0]){if(e.target.files[0].size>5000000){$('#cn-transfer-status').textContent='Слишком большой файл.';return;}importProgress(await e.target.files[0].text());e.target.value='';}});
function renderSite(){if(window.gameStorageIssue)$('#cn-status').textContent='Браузер не разрешает сохранять данные. Скачайте прогресс перед закрытием.';renderAudio();$('#cn-quiz').hidden=state.view!=='quiz';if(state.view==='quiz')renderQuiz();const end=$('#cn-end');if(!end.hidden&&!end.querySelector('[data-open-quiz]'))end.insertAdjacentHTML('beforeend','<button type="button" class="cn-primary" data-open-quiz>Проверить понимание главы →</button>');}

 restore(window.openai?.widgetState);state.chapterQuiz=validatedQuiz(state.chapterQuiz);quizLevel=[0,1,2].includes(state.quizLevel)?state.quizLevel:state.level;quizChapter=availableChapters().includes(state.quizChapter)?state.quizChapter:chapterOf(getScene());render();root.dataset.ready='true';
